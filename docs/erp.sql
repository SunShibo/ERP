...
INSERT INTO `jsh_functions` VALUES ('234', '000105', '系统配置', '0001', '/pages/manage/systemConfig.html', '\0', '0165', '', '电脑版', '', 'icon-notebook', '1');
...
-- Records of jsh_log
-- ----------------------------
INSERT INTO `jsh_log` VALUES ('1', '1', '用户', '0:0:0:0:0:0:0:1', '2019-12-16 03:05:31', '0', '登录,id:1用户', null, '1');
INSERT INTO `jsh_log` VALUES ('2', '1', '用户', '0:0:0:0:0:0:0:1', '2019-12-16 03:07:26', '0', '登录,id:1用户', null, '1');
...
CREATE TABLE `jsh_tenant` (
  ...
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8 COMMENT='租户';

-- ----------------------------
-- Records of jsh_tenant
-- ----------------------------
INSERT INTO `jsh_tenant` VALUES ('1', '1', '666666', '2', '200', '2019-12-16 03:05:18');
...
-- Records of jsh_user
-- ----------------------------
INSERT INTO `jsh_user` VALUES ('1', '系统', '666666', '73fee192744f842a611a5ec336459f5c', null, null, null, null, '1', '0', '0', null, null, '1');
...
INSERT INTO `jsh_userbusiness` VALUES ('69', 'UserRole', '1', '[10]', null, '0');

-- ----------------------------
-- View structure for test
-- ----------------------------
DROP VIEW IF EXISTS `test`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%`  VIEW `test` AS select sum(if(dh.type='入库'  , di.BasicNumber,0)) as inTotal,
        sum(if(dh.SubType='调拨' ,di.BasicNumber,0)) as transfInTotal,
        sum(if(dh.SubType='调拨' ,di.BasicNumber,0)) as  transfOutTotal,
        sum(if(dh.type='出库' and dh.SubType!='调拨' ,di.BasicNumber,0)) as outTotal,
        sum(if(dh.SubType='组装单' and di.MType='组合件' ,di.BasicNumber,0)) as assemInTotal,
        sum(if(dh.SubType='组装单' and di.MType='普通子件' ,di.BasicNumber,0)) as assemOutTotal,
        sum(if(dh.SubType='拆卸单' and di.MType='普通子件' ,di.BasicNumber,0)) as disAssemInTotal,
        sum(if(dh.SubType='拆卸单' and di.MType='组合件' ,di.BasicNumber,0)) as disAssemOutTotal 
        from
        jsh_depothead dh,jsh_depotitem di
        where 1=1
        and dh.id=di.HeaderId
        and di.MaterialId=586
        and dh.tenant_id=133
        and di.tenant_id=63
